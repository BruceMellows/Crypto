...
inline static unsigned numsize(srcptr x, unsigned n) {
	while (n-- && !x[n])
		;
	return n+1;
}
...
inline static void unpack25519(i25519 x, const k25519 m) {
...
inline static int is_overflow(const i25519 x) {
	return ((x[0] > P26-19) & ((x[1] & x[3] & x[5] & x[7] & x[9]) == P25) &
	                          ((x[2] & x[4] & x[6] & x[8]) == P26)
	       ) | (x[9] > P25);
}

inline static void pack32(unsigned char* dst, int64_t src)
{
...
inline static void pack25519(const i25519 x, k25519 m) {
	int32_t ld = 0, ud = 0;
	int64_t t;
	...
	pack32(&m[ 0], t = ld + X(0) + (X(1) << 26));
	pack32(&m[ 4], t = (t >> 32) + (X(2) << 19));
	pack32(&m[ 8], t = (t >> 32) + (X(3) << 13));
	pack32(&m[12], t = (t >> 32) + (X(4) <<  6));
	pack32(&m[16], t = (t >> 32) + X(5) + (X(6) << 25));
	pack32(&m[20], t = (t >> 32) + (X(7) << 19));
	pack32(&m[24], t = (t >> 32) + (X(8) << 12));
	pack32(&m[28], t = (t >> 32) + ((X(9) + ud) << 6));
}
...
inline static void add25519(i25519 xy, const i25519 x, const i25519 y) {
...
inline static void sub25519(i25519 xy, const i25519 x, const i25519 y) {
	xy[0] = x[0] - y[0];	xy[1] = x[1] - y[1];
	xy[2] = x[2] - y[2];	xy[3] = x[3] - y[3];
	xy[4] = x[4] - y[4];	xy[5] = x[5] - y[5];
	xy[6] = x[6] - y[6];	xy[7] = x[7] - y[7];
 ...
 * to the same buffer. */
inline static i25519ptr mul25519small(i25519 xy, const i25519 x, const int32_t y) {
	register int64_t t;
 ...
 * be. */
inline static i25519ptr mul25519(i25519 xy, const i25519 x, const i25519 y) {
	register int64_t t;
...
inline static i25519ptr sqr25519(i25519 x2, const i25519 x) {
	register int64_t t;
...
inline static int is_negative(i25519 x) {
	return (is_overflow(x) | (x[9] < 0)) ^ (x[0] & 1);
}

/* a square root */
inline static void sqrt25519(i25519 x, const i25519 u) {
	i25519 v, t1, t2;
...
/* t1 = ax + az
 * t2 = ax - az  */
inline static void mont_prep(i25519 t1, i25519 t2, i25519 ax, i25519 az) {
	add25519(t1, ax, az);
	sub25519(t2, ax, az);
}
...
inline static void mont_add(i25519 t1, i25519 t2, i25519 t3, i25519 t4,
 ...
 *  X(Q) = (t3+t4)/(t3-t4)
 * clobbers t1 and t2, preserves t3 and t4  */
inline static void mont_dbl(i25519 t1, i25519 t2, i25519 t3, i25519 t4,
			i25519 bx, i25519 bz) {
	sqr25519(t1, t3);
	sqr25519(t2, t4);
	mul25519(bx, t1, t2);
 ...
 * t is a temporary  */
inline static void x_to_y2(i25519 t, i25519 y2, const i25519 x) {
	sqr25519(t, x);
